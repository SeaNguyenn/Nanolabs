  ...
  <div class="bg-white p-4 rounded-lg shadow animate-fade-in-down">
    <div class="flex justify-between border-b-2 pb-3">
      <div class="flex items-center">
        ...
        <select @change="getDataProducts()" v-model="perPage"
        ...
        <span class="ml-3">Tổng có {{ products.total }}</span>
      </div>
      <div>
        <input v-model="search" @change="getDataProducts()"
    ...
    <Table>
      ...
      <table-body v-if="loading || !products.data">
        <table-row>
          <table-cell>
    ...
    <div v-if="!loading" class="flex justify-between items-center mt-5">
      <div v-if="products.data">
        Hiển thị từ {{ products.from }} đến {{ products.to }}
      </div>
...
function deleteProduct(product) {
  ...
  productStore.deleteProduct(product.id).then(res => {
    productStore.fetchProducts()
  })
}
