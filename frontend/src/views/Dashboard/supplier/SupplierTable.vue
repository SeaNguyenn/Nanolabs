  ...
  <div class="bg-white p-4 rounded-lg shadow animate-fade-in-down">
    <div class="flex justify-between border-b-2 pb-3">
      <div class="flex items-center">
        ...
        <select @change="getDataSuppliers()" v-model="perPage"
        ...
        <span class="ml-3">Tổng có {{ suppliers.total }}</span>
      </div>
      <div>
        <input v-model="search" @change="getDataSuppliers()"
    ...
    <Table>
      <table-head>
        <TableHeaderCell field="id" :sort-field="sortField" :sort-direction="sortDirection" @click="sortSuppliers('id')">
          ID
        </TableHeaderCell>
      ...
      <table-body v-if="loading || !suppliers.data">
        <table-row>
          <table-cell>
    ...
    <div v-if="!loading" class="flex justify-between items-center mt-5">
      <div v-if="suppliers.data">
        Hiển thị từ {{ suppliers.from }} đến {{ suppliers.to }}
      </div>
      <nav v-if="suppliers.total > suppliers.limit"
        class="relative z-0 inline-flex justify-center rounded-md shadow-sm -space-x-px" aria-label="Pagination">
        <a v-for="(link, i) of suppliers.links" :key="i" :disabled="!link.url" href="#" @click="getForPage($event, link)"
          aria-current="page"
...
function deleteSupplier(supplier) {
  ...
  supplierStore.deleteSupplier(supplier.id).then(res => {
    supplierStore.fetchSuppliers()
  })
}
