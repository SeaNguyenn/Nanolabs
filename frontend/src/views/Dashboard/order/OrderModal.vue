              ...
              <div class="modal-content overflow-y-auto h-full">
                ...
                <a-form enctype="multipart/form-data" @submit="onSubmit">
                  <div class="flex flex-wrap gap-4 bg-white px-4 pt-5 pb-4">
                    <CustomInput class="mb-2" v-model="order.name" label="Tên sản phẩm" />
                    <CustomInput class="mb-2" v-model="order.supplier_id" label="Mã nhà cung cấp" />
                    <CustomInput class="mb-2" v-model="order.code" label="Mã sản phẩm" />
                    <CustomInput class="mb-2" type="number" v-model="order.price" label="Giá" />
                    <CustomInput class="mb-2" type="number" v-model="order.sale_price" label="Giá Sale" />
                    <CustomInput class="mb-2" type="number" v-model="order.warranty" label="Bảo hành" />
                    <CustomInput class="mb-2" type="textarea" v-model="order.description" label="Thông tin" />
                  </div>
...
const order = ref({
  ...
  shipping_method_id: props.order.shipping_method_id,
  order_status: props.order.order_status,
  order_date: props.order.order_date,
  shipped_date: props.order.shipped_date,
  required_date: props.order.required_date,
})
...
const onSubmit = async () => {
  ...
  if (order.value.id) {
    try {
      await orderStore.updateOrderStatusAndShipper(order.value.id, order.value).then(res => {
        closeModal()
        loading.value = false
    ...
    try {
      await orderStore.addOrder(order.value).then(res => {
        closeModal()
        loading.value = false
        orderStore.fetchOrders()
      });
